...
@app.on_message(filters.command(["chatgpt", "ai", "emini", "ask"], prefixes=["!", ".", "/", "-", "g", "$", "G", "&"]))
async def chat_gpt(app, message):
    try:
        await app.send_chat_action(message.chat.id, ChatAction.TYPING)

        if len(message.command) < 2:
            await message.reply_text("Hᴇʟʟᴏ Sɪʀ/Mᴀ'ᴀᴍ, I Aᴍ Pᴇɴɢᴜɪɴ Aʀᴛɪғɪᴄɪᴀʟ Iɴᴛᴇʟʟɪɢᴇɴᴄᴇ. Hᴏᴡ Cᴀɴ I Hᴇʟᴘ Yᴏᴜ Tᴏᴅᴀʏ??")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            response = g4f.ChatCompletion.create(
...
async def chat_annie(app, message):
    try:
        ...
        name = message.from_user.first_name
        if len(message.command) < 2:
            await message.reply_text(f"Hᴇʟʟᴏ {name}, Sɪʀ/Mᴀ'ᴀᴍ, I Aᴍ Pᴇɴɢᴜɪɴ Aʀᴛɪғɪᴄɪᴀʟ Iɴᴛᴇʟʟɪɢᴇɴᴄᴇ. Hᴏᴡ Cᴀɴ I Hᴇʟᴘ Yᴏᴜ Tᴏᴅᴀʏ?")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            response = g4f.ChatCompletion.create(
